...
class runPfamscan:
    ...
    path = infoFile['Path']
    appsData = ''
    url = 'http://www.ebi.ac.uk/Tools/services/rest/pfamscan'
    jobId = ''
    ...
    def getResult(cls, name):
        ...
        for resultType in resultTypes:
            try:
                if str(resultType['identifier']).strip() == 'out':
                    # Derive the filename for the result
                    filename = name + '.domain'
                    # Get the result
                    result = cls.serviceGetResult(cls.jobId, str(resultType['identifier']))
                    if (str(resultType['mediaType']) == "image/png" or str(
                            resultType['mediaType']) == "image/jpeg") or str(
                            resultType['mediaType']) == "application/gzip":
                        fmode = 'wb'
                    else:
                        fmode = 'w'
                    # Write a result file
