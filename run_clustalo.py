...
class runClustalO:
    ...
    url = 'http://www.ebi.ac.uk/Tools/services/rest/clustalo'
    jobId = ''
    ...
    def getResult(cls, name):
        ...
        for resultType in resultTypes:
            try:
                if str(resultType['identifier']).strip() == 'phylotree':
                    # Derive the filename for the result
                    filename = name + '_tree.dnd'
                    # Get the result
                    result = cls.serviceGetResult(cls.jobId, str(resultType['identifier']))
                elif str(resultType['identifier']).strip() == 'pim':
                    # Derive the filename for the result
                    filename = name + '_identity.ide'
                    # Get the result
                    result = cls.serviceGetResult(cls.jobId, str(resultType['identifier']))
